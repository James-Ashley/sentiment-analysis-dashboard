...
d3.json('api/bigrams/headlines').then( function(data) {
    ...
    var links = svg
        ...
        .attr('d', function (d) {
            ...
            return ['M', 100, start,    // the arc starts at the coordinate 30, start (where the starting node is)
                ...
                (start - end)/2, 0, 0, ',',
                start < end ? 1 : 0, 100, ',', end] // We always want the arc on top. So if end is before start, putting 0 here turns the arc upside down.
                .join(' ');
        })
        .style("fill", "none")
        .attr("stroke", "black")
